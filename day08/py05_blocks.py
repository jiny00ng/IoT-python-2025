...
class Block:
    def __init__(self, col, rect, speed = 0):
        ...
        self.rect = rect
        self.speed = speed
        # 90이면 위로, 270이면 공이 아래로 -45~45 편차로
        self.dir = random.randint(-45, 45) + 90    

    def move(self): # 볼 무브   
        # 볼이 움직이는 x축 값을 계속 계산하려면 x축은 dir 값을 rad으로 변환 후 cos 처리
        self.rect.centerx += math.cos(math.radians(self.dir)) * self.speed
        # 볼이 움직이는 y축 값을 계속 계산하려면 y축은 dir 값을 rad으로 변환 후 sin 처리
...
def main():
    ...
    PADDLE = Block((200,200,0),Rect(375,700,100,30))    # 공을 맞출 패달 생성

    # 클래스 생성
    # 무지개 색 정보
    colors = [(225,0,0), (255,150,0), (255,228,0), 
              (11,201,4), (0,80,225), (0,0,147),
              (201,0,167)]
    # 초기에 생성될 블럭들(무지개 색으로 9개씩, 54개 블록을 생성)
    ...
    M_GAME_SUBTITLE = bigFont.render('PRESS SPACE_BAR', True, 'white')
    ...
    while True:
        ...
        if is_game_start == False:
            ...
            Surface.blit(M_GAME_SUBTITLE, ((SCREEN_WIDTH / 2) - (500 / 2),
                                           (SCREEN_HEIGHT / 2) + 50))
        else:  # 게임시작 후 블록 생성 후 볼이 움직이도록 처리, 바도 움직이도록 처리
            ...
            if len(BLOCK) != LenBlock: # 공이 블럭에 맞아서 블럭이 사라짐
            ...
            if BALL.rect.centerx < 10 or BALL.rect.centerx > (SCREEN_WIDTH - 10): # 게임화면 양쪽 벽 밖을 못나가게
            ...
            if len(BLOCK) == 0: # 볼로 블럭을 다 없앴음.
                Surface.blit(M_CLEAR, ((SCREEN_WIDTH / 2) - (400 / 2),
                                        (SCREEN_HEIGHT /2) - (50 / 2)))
                
            if BALL.rect.centery > 800:
                Surface.blit(M_FAIL, ((SCREEN_WIDTH / 2) - (300 / 2),
                                        (SCREEN_HEIGHT /2) - (50 / 2)))
