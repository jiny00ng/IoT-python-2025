...
## 2. Exception(실행 중 발생 예외) - 문법오류 수정 후 실행하다가 비정상적으로 종료되는 것
...
## except 예외클래스 as e:
##    예외처리 로직
##    Exception 클래스는 다른 모든 예외 클래스의 조상. Exception만 쓰면 됨ㅎ
## [finally:] - 옵션
##     예외발생 유무와 상관없이 항상 처리해야 할 로직
## try문을 반복해서 사용하지 말 것 - 속도가 느려짐ㅜ
# 디버깅 - 천천히 어디에서 예외(오류)가 발생하는지 확인하기 위해 사용
## F9 - 중단점(Break Point) 표시/해제 기능
## F5 - 디버깅 시작, 중단점까지 실행
## F10 - 한줄 실행. 함수가 있어도 함수를 실행하고 넘어감
## F11 - 한줄 실행. 함수가 있으면 함수(파이썬에서 만든) 안으로 진입(jump)
## Shift + F5 - 디버깅 종료
## 변수 탭 - 현재 변수에 들어 있는 값 표시
...
while True:
    ...
    if op == 'q': # 종료조건
        ...
        try: 
            ...
            print(f'{x} * {y} = {mul(x, y)}')
        except Exception as e: # ValueError, DivisionError 둘 다 처리
            # print('입력 실수. 다시 입력해 주세요.') # 사용자를 위한 에러메시지
            print(f'입력실수 {e}')
        ...
        try: 
            ...
            print('Are you stupid? 0으로 나누지 마시오!')
            print(f'{e}')
