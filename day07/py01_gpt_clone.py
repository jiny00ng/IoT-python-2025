...
# 6. 제미나이 API용 구성
genai.configure(api_key='AIzaSyCQxwLuH3AfsAE5KAH2x61udxSX5BVxi8U') # 신청한 API키 
...
# 12. 아이콘 변경
# ./image/ 경로는 삭제
# root.iconbitmap('chatbot.ico')  # pyinstaller로 설치할 때 해당폴더에 복사하고

# 7. 전체에서 사용할 폰트 지정 -> 나눔고딕
myFont = Font(family='NaunumGothic', size=10)
boldFont = Font(family='NaunumGothic', size=10, weight=BOLD, slant=ITALIC)
...
# 8. 입력창에서 엔터를 치면 keypress 이벤트 처리함수 발생
